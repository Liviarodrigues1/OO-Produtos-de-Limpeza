...
/**
 * Lista de compras com o código único de cada produto
 * 
...
public class Compras {
	...
	public String linha(String caminho, String quantidade, String banheiro, String codigo) {
		String linha = null;
		double totalLinha = 0;
		try {
			...
			while (br.ready()) {
				linha = br.readLine();
				if (linha.startsWith(banheiro)) {
					String aux[] = linha.split("@");
					double valor = Double.parseDouble(aux[2]);
					totalLinha += Double.parseDouble(quantidade) * valor;
					linha = String.format("%-5.5s %-29.29s %4.3s %8.2f %9.2f", codigo, banheiro, quantidade, valor,
							totalLinha);
					break;
				}
			}
